...
circular_buffer_t *get_new_circular_buffer(int *source_array, int len)
{
	int *buffer = (int *)calloc(len, sizeof(int));
	memcpy(buffer, source_array, len * sizeof(int));

	circular_buffer_t *to_return = malloc(sizeof(circular_buffer_t));
...
void resize(circular_buffer_t *circular_buffer, int len)
{
