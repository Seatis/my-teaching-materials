...
#define COMMAND_REMOVE_TODOS    "-rm"
...
int main()
{
    ...
    while(1) {
        gets(command);
        if (strstr(command, COMMAND_ADD_TODOS) != NULL) {
			char todo_name[256];
			...
			if (!error)
				add_todo(&todos, todo_name, 0, prio);
        } else if (strstr(command, COMMAND_LIST_TODOS_BY_PRIO) != NULL) {
            list_todo_by_prio(&todos);
